...
export default async function handler(req, res) {
  ...
  const { to, tempPassword, orgName } = req.body;
  ...
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: `You've been invited to Cerect${orgName ? ` — ${orgName}` : ""}`,
      html: `
        <div style="font-family:'DM Sans',Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#0B1E3D">
          ...
          <h2 style="font-size:20px;font-weight:700;margin-bottom:8px;color:#0F3A52">You've been invited</h2>
          ${orgName ? `<p style="color:#5A6E8A;margin-bottom:20px">You have been added to <strong>${orgName}</strong> on Cerect.</p>` : `<p style="color:#5A6E8A;margin-bottom:20px">You have been given access to the Cerect storage management platform.</p>`}
          <div style="background:#F0F4F8;border-radius:10px;padding:20px;margin:24px 0">
            <p style="margin:0 0 8px;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#5A6E8A">Your login details</p>
            <p style="margin:0 0 4px;font-size:14px"><strong>URL:</strong> <a href="https://cerect.com" style="color:#1A4F72">cerect.com</a></p>
